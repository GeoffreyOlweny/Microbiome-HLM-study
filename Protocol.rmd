...
cran_packages <- c(
  ...
  "caret", "ROCR", "e1071", "gridExtra", "fastDummies"
)
# List of Bioconductor packages
bioc_packages <- c(
  "metadeconfoundR", "DirichletMultinomial", "phyloseq", "DESeq2", "edgeR", "MicrobiotaProcess"
)
...
Genexpert_otu_df <- read.table("Genexpert_OTU.txt", header = TRUE, row.names = 1)
MtbInfectionStatus_otu_df <- read.table("MtbInfectionStatus_OTU.txt", header = TRUE, row.names = 1)
# Load taxonomy table
tax_df <- read.table("Tax.txt", header = TRUE, sep = "\t", row.names = 1, check.names = FALSE)
# Replace all "?" values in tax_df with "Unknown"
tax_df[tax_df == "?"] <- "Unknown"
# Load metadata files
Genexpert_metadata_df <- read.csv(file = "Genexpert_metadata.csv", header = TRUE, row.names = 1)
MtbInfectionStatus_metadata_df <- read.csv(file = "MtbInfectionStatus_metadata.csv", header = TRUE, row.names = 1)
```
...
###Function for generating alpha diversity plots for Xpert_results
```{r}
...
generate_plot <- function(metric) {
  ...
  stats_test <- alpha_metrics_df_Group %>%
...
# Save the alpha diversity plot as a high-resolution image
ggsave(filename = "Figure 1A.jpg", 
...
#### Genexpert results
```{r}
# Compute Bray-Curtis dissimilarity matrix and perform PCoA
...
metadata_beta %>%
  ggplot(aes(x = V1, y = V2, color = Group)) +  # PCoA plot by Group
  theme_classic() + 
  ...
  labs(color = "Genexpert results") +  # Set legend title
...
# Perform PERMANOVA test to compare beta diversity between 'Group' groups
# Compute Bray-Curtis dissimilarity matrix from OTU data
permanova <- adonis2(vegdist(t(Genexpert_otu_df), method = "bray", na.rm = TRUE) ~ as.factor(Group), data = metadata_beta)

# Convert PERMANOVA results to a data frame
permanova <- as.data.frame(permanova)

# Save the data frame to a CSV file
write.csv(permanova, file = "Genexpert_permanova_results.csv", row.names = TRUE)

ggsave(filename = "Figure 2A.jpg", 
       plot = beta_final, 
       width = 10, height = 8, dpi = 600)
```
...
# Save the data frame to a CSV file
write.csv(permanova, file = "Mtb_infection status_permanova_results.csv", row.names = TRUE)

ggsave(filename = "Figure 2B.jpg", 
       plot = beta_final, 
       width = 10, height = 8, dpi = 600)
```
...
genus <- read.table("Genus.txt", header = TRUE, row.names = NULL, sep = "\t")
...
ggsave("Figure 3.jpg", plot = combined_plot, dpi = 600, width = 12, height = 8)
...
# Assign clusters to Dirichilet models
...
# Assign column names corresponding to the number of components
colnames(Dirichlet_multinomial_all) <- c(
  "DMM_k=1", "DMM_k=2", "DMM_k=3", 
  "DMM_k=4", "DMM_k=5", "DMM_k=6"
)

#K=4 had the best fitted Dirichilet component
cluster_result <- as.data.frame(Dirichlet_multinomial_4)
meta_alpha_decon <- cbind(meta_alpha_decon, cluster_result)
meta_alpha_decon$Dirichlet_multinomial_4[meta_alpha_decon$Dirichlet_multinomial_4 == 1] <- "Type 1"
meta_alpha_decon$Dirichlet_multinomial_4[meta_alpha_decon$Dirichlet_multinomial_4 == 2] <- "Type 2"
meta_alpha_decon$Dirichlet_multinomial_4[meta_alpha_decon$Dirichlet_multinomial_4 == 3] <- "Type 3"
meta_alpha_decon$Dirichlet_multinomial_4[meta_alpha_decon$Dirichlet_multinomial_4 == 4] <- "Type 4"
# Add a new column 'smpl' with row names
meta_alpha_decon$smpl <- row.names(meta_alpha_decon)

# Create dummy columns for 'Dirichlet_multinomial_3'
meta_alpha_decon <- fastDummies::dummy_cols(.data = meta_alpha_decon, select_columns = "Dirichlet_multinomial_4")
# Set the row names of 'meta_alpha_decon' to the 'smpl' column
row.names(meta_alpha_decon) <- meta_alpha_decon$smpl

# Remove the 'smpl' column from 'meta_alpha_decon'
meta_alpha_decon$smpl <- NULL

# Remove the 'Dirichlet_multinomial_3' column from 'meta_alpha_decon'
meta_alpha_decon$Dirichlet_multinomial_4 <- NULL
# Order the 'alpha_features' dataframe by row names
alpha_features <- alpha_features[order(rownames(alpha_features)), ]

# Order the 'meta_alpha_decon' dataframe by row names
...
meta_alpha_out <- metadeconfoundR::MetaDeconfound(
  ...
  logfile = here::here("output/", "MetadeconfoundR_feature_alpha.log")  # Path to the log file
)
#Heatmap
...
print(alpha_metaDR1)
ggsave("Figure 4.jpg",alpha_metaDR1, width = 8, height =8,limitsize = FALSE, dpi=600)
...
```{r}
# Step 1: Create cluster results and combine with metadata
# - Convert the Dirichlet_multinomial_4 results to a dataframe
# - Combine the cluster results with the existing metadata
cluster_result <- as.data.frame(Dirichlet_multinomial_4)
metadata_beta <- cbind(Group_metadata_beta, cluster_result)
metadata_beta$Dirichlet_multinomial_4 <- as.character(metadata_beta$Dirichlet_multinomial_4)

# Step 2: Calculate centroids for clusters
# - Calculate the mean (centroid) of V1 and V2 for each cluster
centroids <- aggregate(cbind(V1, V2) ~ Dirichlet_multinomial_4, metadata_beta, mean)

# Step 3: Define custom colors for the plot
# - Set specific colors for the different clusters
ComCol <- c("chartreuse3", "deepskyblue2", "red3", "purple")

# Step 4: Create the main plot with points and ellipses
# - Plot V1 vs. V2 with points colored by cluster
# - Add ellipses and centroids to visualize cluster spread and center
Bcluster <- metadata_beta %>%
  ggplot(aes(x = V1, y = V2, color = Dirichlet_multinomial_4)) +  # Map data to axes and color
  theme_classic() +  # Apply a classic theme for simplicity
  scale_color_manual(values = ComCol) +  # Use custom colors for clusters
  geom_point(aes(color = Dirichlet_multinomial_4), size = 5, alpha = 0.8) +  # Plot points
  xlab("PCo 1") + ylab("PCo 2") +  # Label axes
  labs(color = "Microbial Community types") +  # Set legend heading to "Community types"
  theme(axis.title.x = element_text(size = 13),  # Adjust x-axis title size
  ...
  stat_ellipse(aes(color = Dirichlet_multinomial_4)) +  # Add ellipses for clusters
  geom_point(data = centroids, size = 5, shape = 16, color = "black") +  # Plot centroids
  geom_point(data = centroids, size = 4, shape = 21, fill = "white")  # Add white fill to centroids

# Step 5: Create the x-axis density plot
# - Plot the density of V1, filled by cluster
xdensity <- metadata_beta %>%
  ggplot(aes(x = V1)) +  # Set x-axis to V1
  geom_density(alpha = .5, aes(fill = Dirichlet_multinomial_4, color = Dirichlet_multinomial_4)) +  # Density plot
  scale_fill_manual(values = ComCol) +  # Apply custom fill colors
  scale_color_manual(values = ComCol) +  # Apply custom outline colors
...
# Step 6: Create the y-axis density plot with flipped coordinates
# - Plot the density of V2, filled by cluster, with flipped axes
ydensity <- metadata_beta %>%
  ggplot(aes(V2)) +  # Set x-axis to V2
  geom_density(alpha = .5, aes(fill = Dirichlet_multinomial_4, color = Dirichlet_multinomial_4)) +  # Density plot
  scale_fill_manual(values = ComCol) +  # Apply custom fill colors
  scale_color_manual(values = ComCol) +  # Apply custom outline colors
  theme_classic() +  # Use a classic theme
...
# Step 7: Create a blank plot for layout purposes
# - This plot will be used as a spacer in the final layout
blankPlot <- ggplot() +  
  geom_blank(aes(1, 1)) +  # Create an empty plot
...
# Step 8: Combine the plots into a single grid layout with legends at the bottom
# - Arrange the density plots, beta diversity plot, and blank plot in a grid
Beta_diversity <- 
  cowplot::plot_grid(
    xdensity + theme(plot.margin = unit(c(0, 0, 0, 0), "cm")),  # Add x-axis density plot with no margins
    blankPlot + theme(plot.margin = unit(c(0, 0, 0, 0), "cm")),  # Add blank plot with no margins
    Bcluster + theme(legend.position = "bottom",  # Ensure legend is at the bottom
                     plot.margin = unit(c(0, 0, 0, 0), "cm")),  # Add beta diversity plot with no margins
    ydensity + theme(plot.margin = unit(c(0, 0, 0, 0), "cm")),  # Add y-axis density plot with no margins
    nrow = 2,  # Arrange plots in 2 rows
    rel_widths = c(1, 0.5),  # Set relative widths for columns
    rel_heights = c(0.5, 1),  # Set relative heights for rows
    align = "hv"  # Align plots horizontally and vertically
  )

# Step 9: Display the final combined plot
# - This is the final output combining all the components
print(Beta_diversity)
ggsave("Figure 5.jpg",Beta_diversity, width = 8, height =8,limitsize = FALSE, dpi=600)
...
phyloseq_object <- Genexpert_phyloseq 
...
ggsave("Figure 7A.jpg", plot = combined_plot, width = 12, height = 6, dpi = 600)  
```
...
# Save the combined plot as a high-resolution JPG file
ggsave("Figure 7B.jpg", plot = combined_plot, width = 12, height = 6, dpi = 600)  
```
...
# Save the combined plot as a high-resolution JPG file
ggsave("Figure 7C.jpg", plot = combined_plot, width = 12, height = 6, dpi = 600)  
```
#####Genus
```{r}
# Assign the phyloseq object to a variable
...
# Save the combined plot as a high-resolution JPG file
ggsave("Figure 7D.jpg", plot = combined_plot, width = 12, height = 6, dpi = 600)  
```
...
for (i in 1:dim(contrast_combinations)[2]) {
    ...
    result_table <- cbind(result_table, padj = p.adjust(result_table$PValue, method = "fdr"))  # Adjust p-values
    ...
    result_table$level <- as.factor(ifelse(as.vector(result_table$sig) == 1, "enriched", 
                                           ifelse(as.vector(result_table$sig) == -1, "depleted", "nosig")))  # Define significance levels
    ...
    plot_list[[i]] <- ggplot(filtered_table, aes(x = logFC, y = -log2(p), color = level)) +
        geom_point(size = 3, shape = 16) +  # Increase point size and use solid circles
        geom_hline(yintercept = -log10(0.2), linetype = "dashed", color = 'black', size = 0.7) +  # Dashed line for p-value threshold
        geom_vline(xintercept = c(-1, 1), linetype = "dashed", color = 'black', size = 0.7) +  # Dashed lines for fold change thresholds
        ggrepel::geom_text_repel(data = top_taxa, aes(x = logFC, y = -log2(p), label = Genus), size = 4, box.padding = 0.5) +  # Adjust text size and padding
        scale_color_manual(values = c('blue2', 'red2', 'gray50')) +  # Slightly lighter gray for 'nosig'
        labs(title = contrast_name, x = "Log Fold Change", y = "-Log2 P-Value") +  # Descriptive axis labels
...
# Process normalized counts
count_matrix <- as.matrix(count_matrix)
normalized_counts <- t(t(count_matrix) / colSums(count_matrix))  # Normalize counts by sample
normalized_counts_df <- normalized_counts %>%
    t() %>% as.data.frame()  # Convert to dataframe

sample_split <- split(normalized_counts_df, as.factor(sample_data$SampleType))  # Split data by sample type
sample_means <- lapply(sample_split, function(x) colMeans(x))  # Calculate mean counts per sample type
mean_counts_df <- do.call(rbind, sample_means)  # Combine means into dataframe
mean_counts_df <- t(mean_counts_df)

normalized_counts_summary <- as.data.frame(mean_counts_df)  # Convert to dataframe
combined_results <- as.data.frame(combined_results)  # Convert to dataframe
...
ggsave(filename = "Figure 8.jpg", plot = combined_plot, width = 15, height = 10, dpi = 600)
...
# Function to create a bar plot for LEfSe results
...
MicroRoc <- function(otu = NULL, tax = NULL, map = NULL, tree = NULL,
                     ps = NULL, group_var = "Group", repnum = 5) {
  ...
  data_for_modeling$group <- factor(sample_mapping[[group_var]])
  colnames(data_for_modeling) <- paste("OTU", colnames(data_for_modeling), sep = "")
  
  # Random Forest Model
  data_for_modeling <- dplyr::select(data_for_modeling, OTUgroup, everything())
  train_data <- data_for_modeling
  folds <- createFolds(y = train_data[, 1], k = repnum)
  auc_scores <- c()
  
  fold_classes_rf <- as.numeric()  # True class labels for Random Forest
  fold_predictions_rf <- as.numeric()  # Model predictions for Random Forest
  
  for (i in 1:repnum) {
    test_fold <- train_data[folds[[i]], ]
    train_fold <- train_data[-folds[[i]], ]
    
    colnames(test_fold) <- gsub("-", "_", colnames(test_fold)) 
    colnames(train_fold) <- gsub("-", "_", colnames(train_fold)) 
    
    rf_model <- randomForest(OTUgroup ~ ., data = train_fold, importance = TRUE, proximity = TRUE)
    rf_predictions <- predict(rf_model, newdata = test_fold, type = "prob")
    
    fold_classes_rf <- append(fold_classes_rf, as.factor(test_fold$OTUgroup))
    fold_predictions_rf <- append(fold_predictions_rf, rf_predictions[, 2])
  }
  
  # Plotting ROC curve for Random Forest
  rf_pred <- prediction(fold_predictions_rf, fold_classes_rf)  
  rf_perf <- performance(rf_pred, "tpr", "fpr") 
  rf_x <- unlist(rf_perf@x.values)  
  rf_y <- unlist(rf_perf@y.values)
  rf_plot_data <- data.frame(x = rf_x, y = rf_y)
  
  auc_scores[1] <- paste("RF AUC:", round(performance(rf_pred, 'auc')@y.values[[1]], 3), sep = " ")
  
  rf_plot <- ggplot(rf_plot_data) + 
    geom_path(aes(x = x, y = y, colour = x), size = 1, color = "red") + 
    labs(x = "False Positive Rate", y = "True Positive Rate") + 
    annotate("text", x = 0.75, y = 0.5, label = paste("Red: ", auc_scores[1], sep = ""))
  
  # SVM Model
  fold_classes_svm <- as.numeric()  # True class labels for SVM
  fold_predictions_svm <- as.numeric()  # Model predictions for SVM
  
  for (i in 1:repnum) {
    test_fold <- train_data[folds[[i]], ]
    train_fold <- train_data[-folds[[i]], ]
    
    svm_model <- svm(OTUgroup ~ ., data = train_fold, probability = TRUE)
    svm_predictions <- predict(svm_model, newdata = test_fold, decision.values = TRUE, probability = TRUE)
    
    fold_classes_svm <- append(fold_classes_svm, as.numeric(test_fold$OTUgroup))
    fold_predictions_svm <- append(fold_predictions_svm, as.numeric(attr(svm_predictions, "probabilities")[, 2]))
  }
  
  # Plotting ROC curve for SVM
  svm_pred <- prediction(fold_predictions_svm, fold_classes_svm)  
  svm_perf <- performance(svm_pred, "tpr", "fpr") 
  svm_x <- unlist(svm_perf@x.values)  
  svm_y <- unlist(svm_perf@y.values)
  svm_plot_data <- data.frame(x = svm_x, y = svm_y)
  
  auc_scores[2] <- paste("SVM AUC:", round(performance(svm_pred, 'auc')@y.values[[1]], 3), sep = " ")
  
  svm_plot <- rf_plot + 
    geom_path(data = svm_plot_data, aes(x = x, y = y, colour = x), size = 1, color = "blue") + 
    annotate("text", x = 0.75, y = 0.4, label = paste("Blue: ", auc_scores[2], sep = ""))
  
  # GLM Model
  fold_classes_glm <- as.numeric()  # True class labels for GLM
  fold_predictions_glm <- as.numeric()  # Model predictions for GLM
  
  for (i in 1:repnum) {
    test_fold <- train_data[folds[[i]], ]
    train_fold <- train_data[-folds[[i]], ]
    
    glm_model <- glm(OTUgroup ~ ., family = 'binomial', data = train_fold)
    glm_predictions <- predict(glm_model, type = 'response', newdata = test_fold)
    
    fold_classes_glm <- append(fold_classes_glm, test_fold$OTUgroup)
    fold_predictions_glm <- append(fold_predictions_glm, as.numeric(glm_predictions))
  }
  
  # Plotting ROC curve for GLM
  glm_pred <- prediction(fold_predictions_glm, fold_classes_glm)  
  glm_perf <- performance(glm_pred, "tpr", "fpr") 
  glm_x <- unlist(glm_perf@x.values)  
  glm_y <- unlist(glm_perf@y.values)
  glm_plot_data <- data.frame(x = glm_x, y = glm_y)
  
  auc_scores[3] <- paste("GLM AUC:", round(performance(glm_pred, 'auc')@y.values[[1]], 3), sep = " ")
  
  glm_plot <- svm_plot + 
    geom_path(data = glm_plot_data, aes(x = x, y = y, colour = x), size = 1, color = "black") + 
    labs(x = "False Positive Rate", y = "True Positive Rate") +
    annotate("text", x = 0.75, y = 0.3, label = paste("Black: ", auc_scores[3], sep = ""))
  
  # Return the results
  return(list(roc_plot = glm_plot, auc_scores = auc_scores, combined_data = cbind(fold_classes_glm, fold_predictions_glm)))
}
...
# Save the ROC plot as a high-resolution image
ggsave(filename = "Figure 9A.jpg", 
       plot = roc_plot, 
       width = 10, height = 8, dpi = 600)
...
micro_rf_analysis <- function(otu = NULL, tax = NULL, map = NULL, tree = NULL,
  ...
  # Prepare OTU table with scaled data
  ...
  # Set up classification by group and fit a Random Forest model
  ...
  # Extract and format the confusion matrix from the model
  ...
  # Organize confusion matrix and compute model accuracy rate
  ...
  # Create text tables for accuracy and confusion matrix, then combine them into one display
  ...
  # Perform recursive feature elimination cross-validation (RFCV) if requested
  if (perform_rfcv) {
    rfcv_result <- Micro.rfcv(otu = NULL, tax = NULL, map = NULL, tree = NULL, ps = ps, group = "Group", 
                              optimal = top_features, nrfcvnum = 6)
    
    rfcv_plot <- rfcv_result[[1]] # Plot of RFCV results
    rfcv_table <- rfcv_result[[3]] # Table of RFCV results
  } else {
    rfcv_plot <- NULL
    rfcv_table <- NULL
  }
  
  # Identify the most important features (OTUs) based on the Random Forest model
  ...
  # Order features by importance and select the top ones for further analysis
  ordered_features <- dplyr::arrange(feature_importance, desc(MeanDecreaseAccuracy)) %>% as.data.frame()
  row.names(ordered_features) <- ordered_features$feature_id
  ...
  # Merge the top features with their corresponding taxonomy information
  ...
  otu_data <- ggClusterNet::vegan_otu(ps)
  sample_design <- as.data.frame(phyloseq::sample_data(ps))
  
  # Calculate mean abundance of OTUs by group
  group_split <- split(as.data.frame(otu_data), as.factor(sample_design$Group))
  group_means <- lapply(group_split, function(x) colMeans(x, na.rm = TRUE))
  mean_abundance <- do.call(rbind, group_means) %>% t()
  colnames(mean_abundance) <- paste(colnames(mean_abundance), "mean", sep = "")
  
  # Combine the mean abundance data with the top features data
  final_df <- merge(top_features_df, mean_abundance, by = "row.names", all = FALSE)
  head(final_df)
  ...
  # Return the generated plots, RFCV results, top features, and combined tables
  return(list(importance_plot, polar_plot, rfcv_plot, rfcv_table, top_features_df, combined_tables))
}
...
# Save the first plot as a high-resolution image
ggsave(filename = "Figure 9B.jpg", 
       plot = plot1, 
       width = 10, height = 8, dpi = 600)

# Save the second plot as a high-resolution image
ggsave(filename = "Figure 9C.jpg", 
       plot = plot2, 
       width = 10, height = 8, dpi = 600)
