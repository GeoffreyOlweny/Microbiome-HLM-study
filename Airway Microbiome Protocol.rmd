...
# Function to perform recursive feature elimination cross-validation (RFCV)
run_recursive_feature_cv <- function(otu = NULL, tax = NULL, map = NULL, tree = NULL,
  ...
  # Store initial cross-validation error data
  ...
  # Calculate mean error across folds and prepare summary data
  error_cv_df <- error_cv_df[, -1]
  colnames(error_cv_df) <- paste('error_rate', 1:num_folds, sep = '_')
  mean_error <- rowMeans(error_cv_df)
  rfcv_summary <- data.frame(num_features = rfcv_result$n.var, mean_error = mean_error, error_cv_df)
  
  # Transform the data for plotting
  plot_data <- tidyr::gather(rfcv_summary, key = "fold", value = "error_rate", -num_features)
  
  # Generate a plot showing the RFCV results
  rfcv_plot <- ggplot() +
    geom_line(data = plot_data, aes(x = num_features, y = error_rate, group = fold), colour = 'grey') +
    ...
    labs(title = paste('Training set (n =', nrow(otu_table), ')', sep = ''),
         x = 'Number of features', y = 'Cross-validation error rate') +
    annotate("text", x = optimal_features, y = max(rfcv_summary$mean_error), 
             label = paste("optimal = ", optimal_features, sep = ""))
  
  # Return the plot, plot data, and summary data
  return(list(plot = rfcv_plot, plot_data = plot_data, summary_data = rfcv_summary))
}
...
ggsave(filename = "Results/Figure 9C.jpg", 
       plot = plot3, 
       width = 10, height = 8, dpi = 600)
